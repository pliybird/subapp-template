<template>
	<view :style="{height:height,paddingTop:tPadding+'rpx'}" class="refresh-body" @touchmove.stop.prevent>
	  <scroll-view scroll-y  @touchstart.stop="touchstart" @scrolltolower="loadMore" @touchmove.stop="touchmove" @touchend.stop="touchend" class="refresh-scroll">
		  <!-- logo -->
		  <view class="refresh-icon" :style="{top:top+'px',transform: `translateX(-50%) rotate(${rotate_number}deg)`}">
		   ...
		   <view class="content">
				...
				<!-- 上拉加载 -->
				<view class="t-loading-box" v-if="loadingType!=3&&show_more" @click="loadMore">
...
export default {
  ...
  methods: {
	  ...
	  //移动距离
	  touchmove(e){
		  if(!this.is_refreshs&&e.touches.length<=1){
	  ...
	  //结束 
	  touchend(e){
		  if(this.is_refreshs||e.touches.length>=1){return}
		  if(this.move_number>170){
			  this.is_refreshs=true
			  this.$emit('refresh')
